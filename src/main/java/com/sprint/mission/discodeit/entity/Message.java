...
public class Message implements Serializable {
    ...
    public Message(String content, UUID channelId, UUID authorId,List<UUID>attachmentIds) {
        ...
        this.createdAt = Instant.now(); // ^| (Instant.now().getEpochSecond(); 초(s)(단위로) //메시지 생성 시각 기록

        this.content = content; //메시지 내용 저장
        this.authorId = authorId; //누가 쓴 건지
        this.channelId = channelId; //어디에 쓴 건지
        this.attachmentIds = attachmentIds; //첨부파일 목록 저장
    }

    //메시지 내용을 바꾸는 update함수 (수정 시 update도 갱신)
    public void update(String newContent) {
        boolean anyValueUpdated = false; //실제 수정 여부를 판단하기 위한 플래그 변수
        ...
        if (anyValueUpdated) { //실제 수정 발생 시 수정시간을 현재 시각으로 변경
            this.updatedAt = Instant.now();//객체를 밀리초 단위로 변환 (예 1970-01-01 00:00:00 UTC 의미함 (1749636000123))
