...
CREATE TABLE users(
 ...
 CONSTRAINT pk_users PRIMARY KEY (id), --제약 조건 이름 pk_users/ id컬럼을 기본키로 지정.
 CONSTRAINT fk_profile FOREIGN KEY (profile_id)--제약 조건 이름 fk_profile/
                                         --users(이 테이)의 profile_id컬럼을 외래키로 지정.
     REFERENCES binary_contents(id) ON DELETE SET NULL
     --REFERENCES binary_contents(id) : binary_contents테이블의 id컬럼을 가리킴
     --binary_contents에서 해당 row(행/profile_id가 참조하는 행)가 삭제되면
...
CONSTRAINT pk_user_statuses PRIMARY KEY (id), --제약 조건 이름: pk_user_statuses/PK = id 컬럼
